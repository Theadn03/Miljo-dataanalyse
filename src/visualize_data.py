...
def plot_environmental_factors(df):
    ...
    for location in df["Location"].unique():
        ...
        plt.plot(subset["Time"], subset["Humidity_percent"], label=location)
        plt.xlabel("Time")
...
def plot_precipitation(df):
    ...
    for location in df["Location"].unique():
        subset = df[df["Location"] == location]
        plt.plot(subset["Time"], subset["Precipitation_mm"], label=location)
        plt.xlabel("Time")
...
def plot_wind_speed(df):
    ...
    for location in df["Location"].unique():
        subset = df[df["Location"] == location]
        plt.plot(subset["Time"], subset["WindSpeed_mps"], label=location)
        plt.xlabel("Time")
